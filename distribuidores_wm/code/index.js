...
function listarSinPrecio() {
    ...
    fetch("./apis/listarSinPrecio.php")
        ...
        .then((datas) => {
            ...
            for (i in datas) {
                if (i == actualPage) {
                    productos.innerHTML += `<div style="margin-bottom:15px; display:inline-block;" id="page${i}" class="gridCont actualPage">${datas[i]}</div>`;
                } else {
                    productos.innerHTML += `<div style="margin-bottom:15px; display:inline-block;" id="page${i}" class="gridCont visually-hidden">${datas[i]}</div>`;
                }
            }
            pages.innerHTML = `${actualPage + 1} de ${datas.length}`;
            maxPages = datas.length;
